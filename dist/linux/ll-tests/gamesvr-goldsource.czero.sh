...
#####################################################################################################
### CONFIG VARS #####################################################################################
declare LLTEST_CMD="/app/hlds_run -norestart -insecure +sv_lan 1 -game czero +map as_oilrig +maxplayers 16;";
declare LLTEST_NAME="czero-$(date '+%H%M%S')";
#####################################################################################################
#####################################################################################################
...
#####################################################################################################
### TESTS ###########################################################################################
# Stock cstrike condition zero server tests
should_lack 'Running the dedicated server as root' 'Server is not running under root';
should_have 'Setting breakpad minidump AppID = 80' 'Steam App ID 80 is running';
should_have 'Stdio (czero)' 'counter-strike condition zero mod loaded';
...
should_echo "say STARTING COMMAND TESTS" '<Condition Zero> STARTING COMMAND TESTS';
#####################################################################################################
#####################################################################################################
